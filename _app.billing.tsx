...
import type { Coupon, Order, OrderItem, OrderSource, PaymentMethod } from "@/lib/types";
...
function BillingPanel() {
  ...
  const { products, coupons, settings } = useStore();
  ...
  const appliedCoupon: Coupon | null = useMemo(() => {
    const c = coupons.find((c) => c.active && c.code.toLowerCase() === couponCode.trim().toLowerCase());
    return c ?? null;
  }, [couponCode, coupons]);
  ...
  const couponDiscount = appliedCoupon
    ? appliedCoupon.type === "flat"
      ? appliedCoupon.value
      : (appliedCoupon.value * subtotal) / 100
    : 0;
                      ...
                      {filteredCatalog.length === 0 ? (
                        ...
                        filteredCatalog.map((p) => (
                          <button
                            ...
                            className="w-full flex items-center justify-between gap-2 py-2 px-2 rounded hover:bg-accent text-left"
                          >
        ...
        <div className="space-y-4">
          <Card className="lg:sticky lg:top-20">
            ...
            <CardContent className="space-y-4 text-sm">
              ...
              <div>
                <div className="text-[10px] tracking-widest text-muted-foreground font-semibold mb-1 flex items-center gap-1">
                ...
                {appliedCoupon && (
                  <div className="mt-1 text-xs text-success">
                    ✓ {appliedCoupon.code} applied — {inr(couponDiscount)} off
                  </div>
                )}
                {couponCode && !appliedCoupon && (
                  <div className="mt-1 text-xs text-destructive">Invalid coupon</div>
                )}
              </div>
